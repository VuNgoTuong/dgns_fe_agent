<template lang="">
    <!-- Modal -->
    <div class="modal fade" id="CreateAccount_Modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content p-3 shadow-lg rounded-4 border-top border-start-0 border-end-0 border-bottom-0 border-5" style="border-color: #4cc4b3!important;">
                <div class="modal-body p-4">
                    ...
                    <div class="mb-3">
                        <h2 class="modal-title fs-5 fw-bold flex-grow-1" >Tạo người dùng mới</h2>
                        <p>Tạo người dùng mới dựa trên dữ liệu được nhập vào</p>
                    </div>
                    ...
                    <div class="d-flex gap-3 mt-5">
                        ...
                        <button type="button" class="btn bg-gradient-success border-0 text-white w-50 btn-hvr-success" @click="CreateData">Tạo người dùng mới</button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    mounted(){
        modal = new bootstrap.Modal('#CreateAccount_Modal');
    },
    ...
    methods : {
        ...
        CreateData(){
            ...
            params.is_type_account = this.authorize.is_type_account + 1
            let config = {headers: {Authorization: `Bearer ${this.authorize.token}`}}
            axios.post(PATH_API_USER + '/user/create',params,config)
            .then(async res =>{
                modal.hide()
            ...
            .catch(err => {
                ShowToast({status_code : 401, message : 'Tạo người dùng thất bại'})
                console.error(err); 
            })
        },
        ClearData(){this.data = {}}
