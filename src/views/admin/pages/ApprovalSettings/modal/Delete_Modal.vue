<template lang="">
    <div class="modal fade" id="DeleteApprover_Modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
export default {
    ...
    components : {...Icons},
    mounted(){
        modal = new bootstrap.Modal('#DeleteApprover_Modal');
    },
    methods : {
        async _Init(value){
        ...
        DeleteData(){
            ...
            axios.post(PATH_API_USER + '/user-approve/delete',params,config)
            .then(async res =>{
                modal.hide()
                this.$emit('ReloadData')
                ShowToast({status_code : 200, message : 'Xóa phê duyệt thành công'})
            })
            .catch(err => {
                ShowToast({status_code : 401, message : 'Xóa phê duyệt thất bại'})
                console.error(err); 
            })
        }
