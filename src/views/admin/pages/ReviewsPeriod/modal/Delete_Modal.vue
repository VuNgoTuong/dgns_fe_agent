<template lang="">
    <div class="modal fade" id="Delete_ReviewsPeriod" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content p-3 shadow-lg rounded-4 border-top border-start-0 border-end-0 border-bottom-0 border-5" style="border-color:#ff4959!important">
                <div class="modal-body p-4 text-center">
                    ...
                    <div class="d-flex gap-3">
                        ...
                        <button type="button" class="btn bg-gradient-danger text-white w-50 shadow-gradient btn-hvr-danger" @click="DeleteData">Xóa dữ liệu</button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    components : {...Icons},
    mounted(){
        modal = new bootstrap.Modal('#Delete_ReviewsPeriod');
    },
    methods : {
        async _Init(value){
        ...
        DeleteData(){
            const params = {item : this.init.id}
            let config = {headers: {Authorization: `Bearer ${this.authorize.token}`}}
            axios.post(PATH_API_DATA + '/period-review/delete',params,config)
            .then(res => {
                ShowToast({status_code : 200, message : 'Xóa kỳ đánh giá thành công'})
                this.$emit('ReloadData')
                modal.hide()
            })
            .catch(err => {
                ShowToast({status_code : 401, message : 'Xóa kỳ đánh giá thất bại'})
                console.error(err); 
            })
        }
    }
