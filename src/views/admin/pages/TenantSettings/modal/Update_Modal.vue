<template lang="">
    <!-- Modal -->
    <div class="modal fade" id="UpdateTenant_Modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content p-3 shadow-lg rounded-4 border-top border-start-0 border-end-0 border-bottom-0 border-5" style="border-color: #0674d8!important;">
                <div class="modal-body p-4">
                    ...
                    <div class="mb-3">
                        <h2 class="modal-title fs-5 fw-bold flex-grow-1" >Cập nhật đơn vị</h2>
                        <p>Cập nhật đơn vị dựa trên dữ liệu được nhập vào</p>
                    </div>
                    ...
                    <div class="d-flex flex-column gap-4 mb-5">
                        ...
                        <div class="d-flex flex-column">
                            ...
                            <el-tree-select
                                ...
                                :disabled="data.tenant_parent_id == ''"
                                :readonly="data.tenant_parent_id == ''"
                            />
                        </div>
                    </div>
                    ...
                    <div class="d-flex gap-3">
                        ...
                        <button type="button" class="btn bg-gradient-primary border-0 text-white w-50 btn-hvr-success" @click="UpdateData">Cập nhật đơn vị</button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    mounted(){
        modal = new bootstrap.Modal('#UpdateTenant_Modal');
    },
    ...
    methods : {
        ...
        GetTenantById(){
            ...
            axios.post(PATH_API_USER + '/tenant/get-by-id',params,config)
            .then(async res =>{
                let data = res.data.data.tenantData
                if(data.tenant_parent_id == '00000000-0000-0000-0000-000000000000'){
                    data.tenant_parent_id = ''
                }
                this.data = data
            })
            .catch(err => {
        ...
        processWaterfall(arr, parentId) {
            ...
            const result = filteredItems.map((item,key) => {
                ...
                return { 
                    ...
                    disabled : item.id == this.init.id ? true : false,
                    children 
                };
            });
            return result;
        ...
        UpdateData(){
            ...
            console.log(params)
            let config = {headers: {Authorization: `Bearer ${this.authorize.token}`}}
            axios.post(PATH_API_USER + '/tenant/update',params,config)
            .then(async res =>{
                modal.hide()
